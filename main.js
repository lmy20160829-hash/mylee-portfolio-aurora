...
(function () {
  ...
  const D = {
    ...
    mgmtTrend: {
      ...
      series: [
        { name: "생산성 지수", color: "#119bd0", values: [72, 75, 79, 83, 88, 92] },
        { name: "직원 만족도", color: "#a668ff", values: [80, 82, 85, 88, 90, 92] }
      ]
    },
    mgmtBars: [
  ...
  function renderHeatmap(box, cfg) {
    ...
    cfg.rows.forEach(([label, vals]) => {
      html += `<div class="hm-row"><div class="hm-label">${label}</div>${vals.map((v, i) => {
        ...
        let bg, fg;
        if (good >= 88) { bg = "rgba(20,164,134,0.20)"; fg = "#5fe3c0"; }
        else if (good >= 80) { bg = "rgba(17,155,208,0.18)"; fg = "#6fd2f7"; }
        else if (good >= 72) { bg = "rgba(240,196,106,0.16)"; fg = "#f0c46a"; }
        else { bg = "rgba(255,107,122,0.16)"; fg = "#ff93a0"; }
  ...
  gdefs.innerHTML = `<defs><linearGradient id="radGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#6a7bff"/><stop offset="100%" stop-color="#4fd7ff"/></linearGradient></defs>`;
